...
namespace Dany
{
    public class InventoryManager : MonoBehaviour, IMonoUpdate
    {
        ...
        [Header("Pickup Settings")] public InventoryItem pickupItem;
        public bool canPickup = false;

        [Header("Hint Settings")] public Camera playerCamera;
        ...
        public Dictionary<InventoryItem.WeaponType, GameObject> decalPrefabs =
            new Dictionary<InventoryItem.WeaponType, GameObject>();
        ...
        private void PlayerDestroy()
        {
            ...
            for (var i = slotItems.Count - 1; i >= 0; i--)
            {
                if (slotItems[i] != null)
                    DropItem(i);
            }
        ...
        private void Shoot(int slotIndex)
        {
                    ...
                    Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore))
            {
                ...
                {
                    GameObject decal = Instantiate(weapon.decalPrefab, hit.point + hit.normal * 0.01f,
                ...
                {
                    Debug.LogWarning(
                        $"Декаль для оружия {weapon.itemName} (тип {weapon.weaponType}) не задана в InventoryItem!");
                }
            ...
            {
                Debug.Log(
                    $"Выстрел из {weapon.itemName}! Патроны: {ammoInMagazine[slotIndex]}/{weapon.magazineSize} | Запас: {reserveAmmo[slotIndex]}");
            }
            else
            {
        ...
        private void ApplyRecoilToCamera()
        {
            ...
            playerCamera.transform.localRotation =
                playerCamera.transform.localRotation * Quaternion.Euler(recoilCurrent);
        }
        ...
        private IEnumerator ReloadRoutine(int slotIndex, InventoryItem weapon)
        {
            ...
            Debug.Log(
                $"Перезарядка: {ammoInMagazine[slotIndex]}/{weapon.magazineSize} | Запас: {reserveAmmo[slotIndex]}");
        }
        ...
        public void Throw(int slotIndex)
        {
            ...
            GameObject grenade = Instantiate(item.throwPrefab, playerTransform.position + Vector3.up * 1f,
                Quaternion.identity);
        ...
        private void UpdateHand()
        {
            ...
            {
                Debug.LogError(
                    "Hand Socket не присвоен! Присвой Transform в руке игрока в инспекторе InventoryManager.");
                return;
            }

            if (slotItems[selectedSlot] != null && slotCounts[selectedSlot] > 0 &&
                slotItems[selectedSlot].handModel != null)
            {
                currentHandItem = Instantiate(slotItems[selectedSlot].handModel, handSocket.position,
                    handSocket.rotation, handSocket);
